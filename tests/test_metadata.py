...
class TestEnvelope(unittest.TestCase):
    ...
    def test_sign_and_verify(self):
        ...
        for (name, key_dict) in self.key_dicts:
            keys_list.append((name, SSlibKey.from_securesystemslib_key(key_dict)))
        ...
        with self.assertRaises(ValueError):
            envelope_obj.verify(keys_list, 4)
        ...
        for (name, key_dict) in new_keys:
            new_keys_list.append((name, SSlibKey.from_securesystemslib_key(key_dict)))
